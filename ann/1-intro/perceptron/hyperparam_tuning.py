...
turn, make the training process much faster.
...
Normally, realword data is structured in hierarchical way, so...
    ...
    - An the same for highest hidden layers and output layer
...
A common practice is the reduce the number neurons through each layer in the network because many low-level features
can coalesce (combine) into far fewer high-level features.

-> Increase the number of neurons for hidden layers untial we start overfitting the training set.
...
--------------------
Most of the time, we use ReLU (or its variants) activation function for hidden layers because it's fater to compute.
